...
class StabilityWaits:
    ...
    @staticmethod
    async def wait_for_connection_ready(page: Page, timeout: int = 10000):
    ...
    @staticmethod
    async def wait_for_game_ready(page: Page, timeout: int = 15000):
...
class TestDataIsolation:
    ...
    @staticmethod
    def create_isolated_game_config(name_prefix: str = "isolated") -> dict:
...
def quarantine(reason: str):
    ...
    def decorator(func):
        return pytest.mark.quarantine(pytest.mark.skip(reason=f"Quarantined: {reason}")(func))
    return decorator


def flaky_on_ci(reason: str = "Flaky on CI"):
    """Decorator to skip tests that are flaky specifically on CI."""
    import os
    def decorator(func):
        if os.environ.get("CI") == "true":
            return pytest.mark.skip(reason=f"Skipped on CI: {reason}")(func)
        return func
    return decorator
...
class BrowserStabilityHelpers:
    ...
    @staticmethod
    async def click_with_retry(page: Page, selector: str, max_attempts: int = 3):
    ...
    @staticmethod
    async def type_with_retry(page: Page, selector: str, text: str, max_attempts: int = 3):
    ...
    @staticmethod
    async def ensure_element_ready(page: Page, selector: str, timeout: int = 5000):
