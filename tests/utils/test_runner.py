...
def run_pytest(args: List[str], env: Optional[dict] = None) -> int:
...
def all(verbose: bool, coverage: bool):
...
def unit(verbose: bool, no_slow: bool):
    """Run unit tests only."""
...
def e2e(verbose: bool, headed: bool, video: bool):
...
def performance():
    ...
    sys.exit(run_pytest(["tests/", "-m", "performance or benchmark", "--benchmark-only"]))
...
def run(test_path: Optional[str], keyword: Optional[str], marker: Optional[str], 
        verbose: bool, capture: bool):
