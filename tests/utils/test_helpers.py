...
def create_mock_mcts(
    ...
    mock_mcts.choose_best_action.return_value = sorted_actions[0][2] if sorted_actions else "*(4,1)"
...
def assert_valid_game_response(response_data: Dict[str, Any]) -> None:
    ...
    """
    required_fields = [
        "game_id", "status", "mode", "player1", "player2", 
        "current_turn", "move_count", "created_at"
    ]
...
def assert_valid_move_response(response_data: Dict[str, Any]) -> None:
    ...
    """
    required_fields = [
        "success", "game_id", "move", "game_status", 
        "next_turn", "next_player_type"
    ]
