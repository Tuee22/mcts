...
class TestWebSocketManagerConnections:
    ...
    async def test_connect_and_register_properly(self, ws_manager: WebSocketManager):
    ...
    async def test_connect_with_notification_broadcast(self, ws_manager: WebSocketManager):
        ...
        notification_sent = any(
            call[0][0].get("type") == "player_connected" 
            for call in calls
        )
        assert notification_sent
    
    async def test_disconnect_with_notification(self, ws_manager: WebSocketManager):
    ...
    async def test_disconnect_removes_empty_game(self, ws_manager: WebSocketManager):
...
class TestWebSocketManagerBroadcasting:
    ...
    async def test_broadcast_move_detailed(self, ws_manager: WebSocketManager):
    ...
    async def test_broadcast_game_state(self, ws_manager: WebSocketManager):
    ...
    async def test_broadcast_game_created_to_all(self, ws_manager: WebSocketManager):
...
class TestWebSocketManagerErrorHandling:
    ...
    async def test_send_json_safe_with_exception(self, ws_manager: WebSocketManager):
        ...
        await ws_manager._send_json_safe(websocket, {"test": "message"}, dead_connections)
        
        # Websocket should be added to dead connections
        assert websocket in dead_connections
    
    async def test_send_json_safe_success(self, ws_manager: WebSocketManager):
    ...
    async def test_broadcast_to_game_cleans_dead_connections(self, ws_manager: WebSocketManager):
    ...
    async def test_broadcast_to_all_with_mixed_connections(self, ws_manager: WebSocketManager):
    ...
    async def test_send_to_player_broadcasts_to_game(self, ws_manager: WebSocketManager):
...
class TestWebSocketManagerDisconnectAll:
    ...
    async def test_disconnect_all_comprehensive(self, ws_manager: WebSocketManager):
    ...
    async def test_disconnect_all_handles_close_exceptions(self, ws_manager: WebSocketManager):
