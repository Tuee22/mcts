...
class MockCorridorsMCTS(CorridorsMCTSInterface):
    ...
    sorted_actions_sequence: List[List[Tuple[int, float, str]]] = field(default_factory=list)
    sorted_actions_side_effect: Optional[Callable[[bool], List[Tuple[int, float, str]]]] = None
    ...
    def get_sorted_actions(self, flip: bool = False) -> List[Tuple[int, float, str]]:
        ...
        if self.sorted_actions_side_effect:
            ...
            if self._sorted_actions_call_count < len(self.sorted_actions_sequence):
                ...
                return self.sorted_actions_sequence[-1] if self.sorted_actions_sequence else []
