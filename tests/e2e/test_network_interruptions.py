    ...
    async def test_slow_network_conditions(
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                await expect(page.locator('[data-testid="game-container"]')).to_be_visible(timeout=20000)
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                for scenario_name, route_pattern in failure_scenarios:
                    ...
                    relevant_errors = [msg for msg in console_messages if 'error' in msg.lower() or 'failed' in msg.lower()]
                    
                    if relevant_errors:
                        print(f"  ✅ Error detected for {scenario_name}: {len(relevant_errors)} messages")
                    else:
                        print(f"  ℹ️ No explicit errors for {scenario_name} (might be handled silently)")
