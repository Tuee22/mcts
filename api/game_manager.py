...
from .models import (
    GameSession, GameStatus, GameMode, PlayerType, Player,
    Move, BoardState, Position, GameSettings, MCTSSettings,
    MoveResponse, AnalysisResult
)
...
class GameManager:
    ...
    def __init__(self):
        ...
        self.matchmaking_queue: List[Dict[str, Any]] = []
        self.ai_move_queue: asyncio.Queue = asyncio.Queue()
    ...
    async def make_move(
        ...
        async with self._lock:
            ...
            # Check for game end
            next_legal_moves = await self._get_legal_moves_internal(game, for_next_player=True)
            if not next_legal_moves or eval_result is not None:
                ...
                game.termination_reason = "checkmate" if not next_legal_moves else "evaluation"
            ...
            if game.status == GameStatus.IN_PROGRESS and next_player.type == PlayerType.MACHINE:
    ...
    async def analyze_position(self, game_id: str, depth: int = 10000) -> Dict[str, Any]:
    ...
    async def get_hint(self, game_id: str, simulations: int = 5000) -> Dict[str, Any]:
    ...
    async def trigger_ai_move(self, game_id: str):
        """Trigger AI to make a move."""
        await self.ai_move_queue.put(game_id)
    
    async def process_ai_moves(self):
        ...
        while True:
            try:
                ...
                game.mcts_instance.ensure_sims(game.settings.mcts_settings.min_simulations)
        ...
        async with self._lock:
            ...
            if self.matchmaking_queue:
                ...
                game = await self.create_game(
                    ...
                    settings=settings or opponent.get("settings")
                )
    ...
    async def leave_matchmaking(self, player_id: str) -> bool:
        ...
        async with self._lock:
            self.matchmaking_queue = [
                p for p in self.matchmaking_queue
    ...
    async def get_player_stats(self, player_id: str) -> Optional[Dict[str, Any]]:
        ...
        wins = sum(
            1 for g in self.games.values()
            if g.is_player(player_id) and g.status == GameStatus.COMPLETED
            and ((g.winner == 1 and g.player1.id == player_id) or
                 (g.winner == 2 and g.player2.id == player_id))
        )
