...
class TypeSafetyChecker(ast.NodeVisitor):
    ...
    def visit_ImportFrom(self, node: ast.ImportFrom) -> None:
        ...
        if node.module and "typing" in node.module:
            for alias in node.names:
                ...
                if name == "Any":
                    self.errors.append(
                        (node.lineno, node.col_offset, "TYP001: Import of 'Any' is forbidden")
                    )
                elif name == "cast":
                    self.errors.append(
                        (node.lineno, node.col_offset, "TYP002: Import of 'cast' is forbidden")
                    )
    ...
    def visit_Name(self, node: ast.Name) -> None:
        ...
        if isinstance(node.ctx, ast.Load):
            if node.id == "Any" and "Any" in self.imported_names:
                self.errors.append(
                    (node.lineno, node.col_offset, "TYP001: Use of 'Any' type is forbidden")
                )
        self.generic_visit(node)
    ...
    def visit_Call(self, node: ast.Call) -> None:
        ...
        if isinstance(node.func, ast.Name) and node.func.id == "cast":
            self.errors.append(
                (node.lineno, node.col_offset, "TYP002: Use of 'cast' function is forbidden")
            )
        self.generic_visit(node)
