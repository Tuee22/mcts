...
const { mockGameStore, mockUseGameStore } = vi.hoisted(() => {
  const store = {
    ...
    dispatch: vi.fn(),
    // New API methods required by components
    getCurrentGameId: vi.fn(() => store.gameId),
    getCurrentGameState: vi.fn(() => store.gameState),
    getSelectedHistoryIndex: vi.fn(() => store.selectedHistoryIndex),
    getLatestError: vi.fn(() => store.error),
    getIsLoading: vi.fn(() => store.isLoading),
