...
BUILD_SURFACE_FILES = {
    ...
    'pyproject.toml', 'setup.cfg', 'setup.py', 'Pipfile', 'poetry.lock', 'Makefile'
}
...
def block_with_reason(stage, reason):
    ...
    print(f"  @{agent_name}")
    print(f"")
    print(f"The {agent_name} agent will handle the {stage.lower()} issues and iterate until resolved.")
...
def main():
    ...
    black_cmd = 'poetry run black .' if os.system('which poetry >/dev/null 2>&1') == 0 else 'black .'
    if not run_command(black_cmd, 'Black'):
        ...
        black_check_cmd = 'poetry run black --check .' if os.system('which poetry >/dev/null 2>&1') == 0 else 'black --check .'
        if not run_command(black_check_cmd, 'Black-Verify'):
            block_with_reason('Black', 'Code formatting verification failed. Files are not properly formatted.')
    
    # Stage 2: MyPy type checking
    # Try poetry first, fallback to direct command
    mypy_cmd = 'poetry run mypy' if os.system('which poetry >/dev/null 2>&1') == 0 else 'mypy'
    ...
    if needs_build:
        if not run_command(BUILD_CMD, 'Build'):
            block_with_reason('Build', 'Docker build failed. Build errors need to be resolved.')
    else:
    ...
    if ALWAYS_TEST or needs_build or changed_files:
        # Try poetry first, fallback to direct command
        test_cmd = 'poetry run pytest -q' if os.system('which poetry >/dev/null 2>&1') == 0 else 'pytest -q'
