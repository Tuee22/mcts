...
def run_git_guard_test(
    command: str, expect_blocked: bool = True
) -> Tuple[bool, str, int]:
    """
    ...
    tool_call = {"parameters": {"command": command, "description": "Test command"}}
...
EOF
        ...
        (
            'echo "setup" && git commit -m "message" && echo "done"',
            "Git commit in command chain",
        ),
    ...
    for command, description in commands_should_be_blocked:
        test_passed, output, return_code = run_git_guard_test(
            command, expect_blocked=True
        )
    ...
    print(
        f"Overall test success rate: {total_passed}/{total_tests} ({100*total_passed/total_tests:.1f}%)"
    )
    ...
    if total_passed == total_tests:
        ...
        print(
            f"⚠️  {total_tests - total_passed} tests failed. Review the failures above."
        )
...
EOF
    ...
    test_passed, output, return_code = run_git_guard_test(
        original_command, expect_blocked=True
    )

    print(
        f"Original bypassing command test: {'✅ BLOCKED' if test_passed else '❌ BYPASSED'}"
    )
    print(f"Return code: {return_code}")
    if output:
